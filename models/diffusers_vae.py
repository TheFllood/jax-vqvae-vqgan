...
class FlaxAttnDownEncoderBlock2D(nn.Module):
    in_channels: int
    out_channels: int
    dropout: float = 0.0
    num_layers: int = 1
    ...
    def setup(self):
        # Prepare the resnet and attention modules
        resnets = []
        attentions = []
        ...
        # Convert lists to Flax Modules
        self.resnets = resnets
        self.attentions = attentions

        # Downsample layer setup if required
        if self.add_downsample:
            self.downsamplers = [
                    FlaxDownsample2D(
                        out_channels=self.out_channels, 
    ...
    def __call__(self, hidden_states, deterministic=True):
        ...
        # Pass through the ResNet and Attention layers
        for resnet, attn in zip(self.resnets, self.attentions):
            hidden_states = resnet(hidden_states, deterministic=deterministic)
            hidden_states = attn(hidden_states)

        # Pass through downsamplers if available
    ...
    def setup(self):
        ...
        for i, down_block_type in enumerate(self.down_block_types):
            ...
            if down_block_type == "DownEncoderBlock2D":
                ...
                down_block = FlaxAttnDownEncoderBlock2D(
                    in_channels=input_channel,
                    out_channels=output_channel,
                    num_layers=self.layers_per_block,
                    resnet_groups=self.norm_num_groups,
